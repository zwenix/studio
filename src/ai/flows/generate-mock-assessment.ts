...
export async function generateMockAssessment(input: GenerateMockAssessmentInput): Promise<GenerateMockAssessmentOutput> {
  const response = await ai.generate({
    ...
    For Grades R–7: use emojis, friendly headings, simple language.
    For Grades 8–12: formal structure, academic language.`,
  ...
  if (!response.output) {
    throw new Error('Mock assessment generation returned no output. Please try again.');
  }
