...
export function ContentCreatorClient() {
  ...
  const handleGenerateTeaching = async () => {
    ...
    if (!t_type || !t_grade || !finalSubject || !finalTopic || !user?.uid) { // Added user?.uid check
    ...
    try {
      const input: GenerateCAPSContentInput = { // Changed type to GenerateCAPSContentInput
        category: t_category,
        contentType: t_type,
        grade: t_grade,
      ...
      // Changed flow call to generateCAPSContent
  ...
  const handleSave = async () => {
    ...
    setIsSaving(true);
    try {
      // The generateCAPSContent flow now directly saves to Firebase,
      // so this `addDoc` is mostly for `visual` and `admin` tabs.
      // For teaching, we rely on the flow to save and return docId.
      ...
      if (activeTab === \'teaching\' && teachingResult) {
        // The teaching flow now returns docId if saved, or we'd get content directly.
        // If the flow handles saving, we don't need to save again here.
        // We will just update the toast message.
      ...
      if (activeTab === \'visual\' || activeTab === \'admin\') { // Only save if not teaching tab
